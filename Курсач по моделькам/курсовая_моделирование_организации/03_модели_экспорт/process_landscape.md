...
|---------|-------------------------------|
| Цифровой заказ, доступность доставки, срок/статус, оценка, рекламация | Маркетинг, найм, закупки, финансы |
...
| Границы процесса | SIPOC | [[sipoc_granitsy_protsessov]] |
...
flowchart TB
    subgraph MAIN["Основные процессы — ценность для клиента"]
        ...
        P3["P3 · Расчёт и сопровождение<br/>срока доставки"]
...
```mermaid
flowchart LR
    subgraph PROB1["Проблема № 1 — доступность и срок"]
        P2p["P2: ближайшая точка<br/>недоступна / отказ"]
        P3p["P3: срок неясен<br/>или риск срыва"]
    end
...
|---------------------|------------|:-------------:|
| Альтернативная пиццерия при перегрузе ближайшей точки | P2/P3 TO BE (ветка A) | нет |
| Проактивный пересчёт срока и push при задержке | P3 TO BE (ветка C) | нет |
...
### P3 · Расчёт и сопровождение срока доставки

| | |
|---|---|
| **Тип** | Основной |
| **Суть (нейтрально)** | Система сопровождает срок доставки для клиента: может показывать расчётное время и/или этапы выполнения заказа, учитывает загрузку точки, курьеров и маршрут (по открытым материалам Dodo IS). |
| **Проблема (диагностика)** | При перегрузе ближайшей точки клиент может получить отказ без альтернативы; при принятом заказе срок может быть неясным или нестабильным, а при риске срыва нет управляемого сценария для клиента — это предмет **TO BE**, не название блока на landscape. |
| **Вход** | Подтверждённый заказ, данные о точке и доставке |
| **Выход** | Заказ принят с понятным сроком/статусом или клиент получил управляемую альтернативу до оплаты |
| **Роль в проекте** | **Главный BPMN** (AS IS + TO BE). Проблема № 1 из [[03_РАНЖИРОВАНИЕ]] |

> [!important] Детализация
> Следующий шаг: [[sipoc_granitsy_protsessov]] → BPMN **AS IS** (реконструкция) → анализ → BPMN **TO BE** (ветки A, C, опционально B).
...
| **Вход** | Принятый заказ с расчётным сроком/статусом |
| **Выход** | Заказ готов к выдаче курьеру |
| **Роль в проекте** | Влияет на соблюдение срока доставки; в **AS IS** связь с P3 может быть слабой — в **TO BE** (ветка C) добавляется проактивный пересчёт |
...
| **Роль в проекте** | Замыкание ожидания по сроку; при срыве ожиданий — переход к **P7** |
...
**P2/P3 (отказ, недоступность или сбой ожиданий по сроку) → P6 (плохой опыт доставки) → P7 (жалоба)**
